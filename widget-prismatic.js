...
cpdefine("inline:org-jscut-widget-prismatic", ["chilipeppr_ready", "Three", "ThreeSTLLoader", "Clipper", "WrapVirtualDom"], function () {
    ...
    return {
        ...
        init: function() {
            ...
            chilipeppr.subscribe("/org-jscut-widget-mesh/removed", this, this.changedMesh);
        },
        ...
        renderBody: function () {
            ...
            return h('table', { style: { width: '100%' } }, [
                h('tr', {}, h('th', {}, 'File')),
                this.meshes.filter(mesh => mesh.types.indexOf('3d') >= 0).map(mesh =>
                    h('tr', {}, [
                        h('td', {}, mesh.name),
                        h('td', { style: { float: 'right' } },
                            h('button',
                                { 'onclick': e => { this.chopMesh(mesh) } },
                                h('span.glyphicon.glyphicon-cutlery')))]))]);
        },
        ...
        chopMesh: function (mesh) {
            ...
            for (let i = 0; i < pos.length; i += 9) {
                if (pos.array[i + 2] === pos.array[i + 5] && pos.array[i + 2] === pos.array[i + 8]) {
                    let dx1 = pos.array[i + 3] - pos.array[i + 0];
                    let dy1 = pos.array[i + 4] - pos.array[i + 1];
                    let dx2 = pos.array[i + 6] - pos.array[i + 0];
                    let dy2 = pos.array[i + 7] - pos.array[i + 1];
                    if (dx1 * dy2 - dx2 * dy1 > 0)
                        indexes.push(i);
                }
            }
            indexes.sort((a, b) => pos.array[a + 2] - pos.array[b + 2]);

            let plane = 0;
            for (let begin = 0; begin < indexes.length;) {
                let z = pos.array[indexes[begin] + 2];
                let end;
                for (end = begin + 1; end < indexes.length && pos.array[indexes[end] + 2] === z; ++end)
                    ;
                let planeGeometry = new THREE.BufferGeometry();
                let planeVertices = new Float32Array(end * 9);
                let k = 0;
                for (let i = 0; i < end; ++i) {
                    let index = indexes[i];
                    planeVertices[k++] = pos.array[index + 0];
                    planeVertices[k++] = pos.array[index + 1];
                    planeVertices[k++] = z;
                    planeVertices[k++] = pos.array[index + 3];
                    planeVertices[k++] = pos.array[index + 4];
                    planeVertices[k++] = z;
                    planeVertices[k++] = pos.array[index + 6];
                    planeVertices[k++] = pos.array[index + 7];
                    planeVertices[k++] = z;
                }
                planeGeometry.addAttribute('position', new THREE.BufferAttribute(planeVertices, 3));
                let planeMaterial = new THREE.MeshBasicMaterial({ color: 0x800000 });
                let planeMesh = new THREE.Mesh(planeGeometry, planeMaterial);
                chilipeppr.publish('/org-jscut-widget-mesh/addThreeMesh', planeMesh, {
                    name: mesh.name + ': Machine ' + plane,
                    types: ['2d'],
                });
                ++plane;
                begin = end;
