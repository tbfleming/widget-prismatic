...
cpdefine("inline:org-jscut-widget-prismatic", ["chilipeppr_ready", "Three", "ThreeSTLLoader", "Clipper", "WrapVirtualDom"], function () {
    ...
    return {
                ...
                h('button',
                    {
                        ...
                        onclick: e => this.chopMesh(this.objectMesh),
                    },
        ...
        chopMesh: function (mesh) {
            let triangles = this.getPlaneTriangles(mesh);
            for (let i = 0; i < triangles.planes.length; ++i) {
                let plane = triangles.planes[i];
                let paths = this.getPathsFromTriangles(triangles, plane.begin, plane.end);
                let planeMesh = this.meshWidget.pathsToThreeMesh(paths, plane.z, this.scale, { color: 0x800000 });
                chilipeppr.publish('/org-jscut-widget-mesh/addThreeMesh', planeMesh, {
                    name: mesh.name + ': Machine ' + i,
                    types: ['2d'],
                });
            }
        },
